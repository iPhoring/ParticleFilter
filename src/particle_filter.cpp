 ...
 * Modified: member function added by Subir Das / Jan 8 2020
 */

// working version Jan 8 2020
...
static default_random_engine gen; //used in multiple functions - as my code is running very slow
...
void ParticleFilter::init(double x, double y, double theta, double std[]) {
  ...
  // checking if the particles are initilized correctly
  // for(unsigned int k=0;k<particles.size();++k){
  //   std::cout<<particles[k].x<<" === "<<particles[k].y<<" ===" <<particles[k].theta<<"\n";
  // }
  is_initialized=true;
}
...
void ParticleFilter::prediction(double delta_t, double std_pos[], 
   ...
   * NOTE: adding noise using std::normal_distribution 
  ...
  for (int i=0;i<num_particles;++i){
    ...
    if (fabs(yaw_rate) <0.00001 ){ //vehicle direction is not changing
      ...
      // adding measurements to each particles
  ...
  // checking if the particles are predicted correctly
  for(unsigned int k=0;k<particles.size();++k){
    std::cout<<particles[k].x<<" === "<<particles[k].y<<" ===" <<particles[k].theta<<"\n";
  }
}
...
void ParticleFilter::dataAssociation(vector<LandmarkObs> predicted, 
  ...
  for(unsigned i=0;i<transformedObs.size();++i){
    ...
    double min_dist=numeric_limits<double>::max(); //sensor_range;
    int landmark_id=-1;
...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[], 
   ...
   */
  for (int i=0;i<num_particles;++i){
    particles[i].transformedObs.clear();
    particles[i].predictionObs.clear();
    ...
    for (unsigned int l=0;l<particles[i].transformedObs.size();++l){
...
void ParticleFilter::resample() {
  ...
  uniform_int_distribution<> disIndex(0, num_particles-1);
  
  int index=disIndex(gen);
  double beta=0.0;
  double max_weight = *max_element(weights.begin(), weights.end());
  uniform_real_distribution<double> disBeta(0.0, max_weight);

  // resampling using the wheel approach
  for (int j=0;j<num_particles;++j){
    beta +=disBeta(gen) *2.0;
